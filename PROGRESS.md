...
| `src/ACTIONS.C` | Ubuntu: `xdg-open` / 直接実行。X68K: `_dos_exec2()`。`action_cd` は共通（`/tmp/xfind_cd` 書き出し） |
...
| `src/UITUI.C` | ANSI エスケープ TUI。raw mode: Ubuntu は `termios`、X68K は `_dos_getchar()` |
